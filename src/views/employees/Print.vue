  ...
  <div id="myPrint" class="dashboard-container">
    <div class="app-container">
      <el-card>
        ...
        <el-button v-print="'printBox'" type="primary" style="float: right">打印</el-button>
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      const { data: { data: userInfo }} = await reqGetUserDetailById(this.userId) // 获取个人基本信息(顶部)
      console.log('上面信息', userInfo)
      const { data: { data: detail }} = await reqGetPersonalDetail(this.userId) // 获取个人基本信息(底部)
      this.formData = { ...detail, ...userInfo }
    },
    async getJobDetail() {
      const { data: { data: userInfo }} = await reqGetUserDetailById(this.userId)
      const { data: { data: jobInfo }} = await reqGetJobDetail(this.userId) // 获取个人基本信息
