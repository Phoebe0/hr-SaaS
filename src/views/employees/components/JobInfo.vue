...
import { reqGetJobDetail, reqUpdateJob, reqGetSimpleList } from '@/api/employees'
...
methods: {
  // 获取基本岗位详情数据
  async getJobDetail() {
    const { data: { data }} = await reqGetJobDetail(this.userId)
    this.formData = data
  },
  // 获取员工列表, 用于将来用户选择上级 (以及, 上级员工名称的显示)
  async getEmployeeSimple() {
    const { data: { data }} = await reqGetSimpleList()
    this.list = data
  },
  async saveJob() {
    await reqUpdateJob({
